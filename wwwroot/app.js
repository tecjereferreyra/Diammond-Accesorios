...
/**
 * Devuelve texto seguro, evitando nulls y undefined
 */
...
/**
 * ⭐ CAMBIO 2: Función normalizar mejorada para manejar singular/plural
 * Normaliza texto: minúsculas, sin acentos, sin espacios extras
 * y convierte plurales a singular para categorías
 */
const normalizar = texto => {
    ...
    // Convertir plurales a singular para unificar categorías
    resultado = resultado
        .replace(/cadenas\s+con\s+dijes/g, 'cadena con dije')
        .replace(/^aros$/g, 'aro')
        .replace(/^anillos$/g, 'anillo')
        .replace(/^pulseras$/g, 'pulsera')
        .replace(/^corbatines$/g, 'corbatin')
        .replace(/^collares$/g, 'collar')
        .replace(/^gargantillas$/g, 'gargantilla')
        .replace(/^dijes$/g, 'dije');
...
function verificarUsuarioAutorizado() {
    ...
    if (usuario) {
        if (domCache.greeting) {
            domCache.greeting.textContent = `¡${usuario}!`;
        }
...
function renderizarProductosProgresivo(cantidad = BLOQUE_CARGA) {
    ...
    for (let i = productosRenderizados; i < limite; i++) {
        ...
        const delay = (i - productosRenderizados) * 0.05;
...
const busquedaDebounced = debounce(ejecutarBusqueda, 300);
...
function abrirModal(producto) {
    ...
    const acciones = `
        <div class="modal-actions-container ${esAdmin ? "" : "solo-whatsapp"}">
            <a href="https://wa.me/5493573446504?text=Hola,Vale!%20Me%20interesa%20el%20producto:%20${encodeURIComponent(safeText(producto.Nombre))}%20-%20${encodeURIComponent(safeText(producto.Modelo))}"
               target="_blank"
    ...
    if (esAdmin) {
        ...
        domCache.modal.querySelector(".btn-admin-edit")?.addEventListener("click", () => {
            cerrarModal();
            setTimeout(() => abrirEditarProducto(producto.IdProducto), 250);
        });

        domCache.modal.querySelector(".btn-admin-delete")?.addEventListener("click", () => {
            cerrarModal();
            setTimeout(() => abrirEliminarProducto(producto.IdProducto, producto.Nombre), 250);
        });
    }
...
async function guardarNuevoProducto() {
    try {
        ...
        formData.append("Categoria", document.getElementById("prodCategoria").value.trim());
        ...
        if (!response.ok) {
            ...
            alert("Error al agregar producto: " + errorText);
            return;
        }
...
async function abrirEditarProducto(id) {
    ...
    await cargarOpcionesDatalistEditar();
    ...
    try {
        ...
        document.getElementById("prodMedidaEditar").value = producto.medida ?? "";
        document.getElementById("prodTalleEditar").value = producto.talle ?? "";
        document.getElementById("prodStockEditar").value = producto.stock ?? "";

        const preview = document.getElementById("imgPreviewEditar");
        if (preview) {
            if (producto.imagenUrl) {
                const imgUrl = producto.imagenUrl.startsWith("http")
                    ? producto.imagenUrl
                    : `${window.location.origin}${producto.imagenUrl}`;
        ...
        if (modal) {
            ...
            setTimeout(() => {
                toggleFieldsByNombre(producto.nombre, "edit");
            }, 0);

            const inputNombre = document.getElementById("prodNombreEditar");
            if (inputNombre) {
                inputNombre.removeEventListener("input", handleNombreChange);
                inputNombre.addEventListener("input", handleNombreChange);
            }
...
async function guardarEdicionProducto() {
    try {
        ...
        formData.append("Categoria", document.getElementById("prodCategoriaEditar").value);
...
function inicializarMenuMovil() {
    ...
    document.querySelectorAll(".mobile-categories li").forEach(item => {
        item.addEventListener("click", () => {
            ...
            const link = document.querySelector(`.categories-vertical a[data-cat="${cat}"]`);
            link?.click();
...
async function handleLogin(e) {
    ...
    try {
        ...
        alert("Error, el correo está en uso.");
    }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (domCache.btnBuscar) {
        domCache.btnBuscar.addEventListener("click", ejecutarBusqueda);
    }
    ...
    // Botón login
    const btnLogin = document.getElementById("openLogin");
    btnLogin?.addEventListener("click", openUserModalAsLogin);

    // Botón logout
    const btnLogout = document.getElementById("logoutBtn");
    btnLogout?.addEventListener("click", mostrarLogoutConfirm);
    ...
    domCache.userModal?.addEventListener("click", e => {
        if (e.target === domCache.userModal) {
            cerrarModalUsuario();
        }
    });

    console.log("✨ Diamonds Accessory cargado correctamente");
});